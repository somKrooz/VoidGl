...
{
    if (ImGui::GetIO().WantCaptureKeyboard) return; // Prevents camera from processing when UI is active
    if (CurrentCamera)
...
{
    if (ImGui::GetIO().WantCaptureMouse) return; // Prevents camera from processing when UI is active
...
Camera::Camera()
{
    ...
    KroozInput::RegisterInputFunction(KeyCallback);
    KroozInput::RegisterMouseFunction(MouseCallback);
}
